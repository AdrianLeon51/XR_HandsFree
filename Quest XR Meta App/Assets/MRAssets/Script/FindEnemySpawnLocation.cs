...
public class FindEnemySpawnLocation : FindSpawnPositions
{
    ...
    private int countSpawnAmount = 0;
    private void Start()
    {
        cameraTransform = GameObject.FindWithTag("MainCamera").transform;
    }

    public void SpawnCoffinPoint(GameObject spawnedObj = null)
    {
        var room = MRUK.Instance.GetCurrentRoom();
        var prefabBounds = Meta.XR.MRUtilityKit.Utilities.GetPrefabBounds(spawnedObj);
        ...
        for (int i = 0; i < SpawnAmount; ++i)
        {
            for (int j = 0; j < MaxIterations; ++j)
            {
                ...
                if (spawnedObj.gameObject.scene.path == null)
                {
                    ...
                    StartCoroutine(SpawnCountDown(spawnTimer, spawnPosition, spawnRotation, spawnedObj));
                }
                else
                {
    ...
    private IEnumerator SpawnCountDown(int seconds, Vector3 spawnPosition, Quaternion spawnRotation, GameObject spawnedObj)
    {
        int counter = seconds;
        while (counter > 0)
        {
        ...
        GameObject spawnedObject = Instantiate(spawnedObj, transform);
